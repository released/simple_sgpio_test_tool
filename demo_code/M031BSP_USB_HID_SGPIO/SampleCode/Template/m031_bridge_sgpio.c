...
#define M031_SGPIO_SDATA_IN_PIN        1u
#define M031_SGPIO_SLOAD_PIN           3u
#define M031_SGPIO_SDATA_OUT_PIN       0u
#define M031_SGPIO_SCLOCK_PIN          2u
#define M031_SGPIO_OUT_MASK            (BIT0 | BIT2 | BIT3)
#define M031_SGPIO_IDLE_OUT_MASK       (BIT3)
#define M031_SGPIO_CAPTURE_WORD_MAX    4u
#define M031_SGPIO_WAVEFORM_WORD_MAX   130u
#define M031_SGPIO_PDMA_CH             0u
#define M031_SGPIO_TIMER               TIMER0
#define M031_SGPIO_TIMER_MODULE        TMR0_MODULE
#define M031_SGPIO_TIMER_CLKSEL        CLK_CLKSEL1_TMR0SEL_HIRC
#define M031_SGPIO_PDMA_REQ            PDMA_TMR0
...
} M031_BRIDGE_SGPIO_STATE_T;

extern uint32_t get_tick(void);

static M031_BRIDGE_SGPIO_STATE_T g_sgpio =
{
    0u,
    0u,
    1u,
...
static uint32_t g_sgpio_waveform[M031_SGPIO_WAVEFORM_WORD_MAX];

static uint32_t BridgeSgpio_ClampClock(uint32_t clock_hz)
{
    if (clock_hz < M031_BRIDGE_SGPIO_CLOCK_MIN_HZ)
    {
        return M031_BRIDGE_SGPIO_CLOCK_MIN_HZ;
    }
    if (clock_hz > M031_BRIDGE_SGPIO_CLOCK_MAX_HZ)
    {
        return M031_BRIDGE_SGPIO_CLOCK_MAX_HZ;
    }
    return clock_hz;
}
...
static uint8_t BridgeSgpio_ClampSlotCount(uint8_t slot_count)
{
    ...
    if (slot_count > M031_BRIDGE_SGPIO_SLOT_MAX)
    {
        return M031_BRIDGE_SGPIO_SLOT_MAX;
    }
    return slot_count;
}
...
{
    if (interval_ms < M031_BRIDGE_SGPIO_INTERVAL_MIN_MS)
    {
        return M031_BRIDGE_SGPIO_INTERVAL_MIN_MS;
    }
    if (interval_ms > M031_BRIDGE_SGPIO_INTERVAL_MAX_MS)
    {
        return M031_BRIDGE_SGPIO_INTERVAL_MAX_MS;
    }
    return interval_ms;
}
...
{
    PA->DOUT = (PA->DOUT & ~M031_SGPIO_OUT_MASK) | M031_SGPIO_IDLE_OUT_MASK;
}
...
static void BridgeSgpio_InitDmaClock(void)
{
    ...
    CLK_EnableModuleClock(M031_SGPIO_TIMER_MODULE);
    CLK_SetModuleClock(M031_SGPIO_TIMER_MODULE, M031_SGPIO_TIMER_CLKSEL, 0u);
    SYS_LockReg();
}
...
static uint8_t BridgeSgpio_IsFrameDone(void)
{
    ...
    if ((PDMA_GET_TD_STS(PDMA) & (1ul << M031_SGPIO_PDMA_CH)) != 0u)
    {
        return 1u;
    }
    if ((PDMA->DSCT[M031_SGPIO_PDMA_CH].CTL & PDMA_DSCT_CTL_OPMODE_Msk) == PDMA_OP_STOP)
    {
        return 1u;
    }
    return 0u;
...
{
    TIMER_Stop(M031_SGPIO_TIMER);
    TIMER_SetTriggerTarget(M031_SGPIO_TIMER, 0u);
    TIMER_ClearIntFlag(M031_SGPIO_TIMER);

    PDMA_PAUSE(PDMA, M031_SGPIO_PDMA_CH);
    PDMA_RESET(PDMA, M031_SGPIO_PDMA_CH);
    PDMA_CLR_TD_FLAG(PDMA, (1ul << M031_SGPIO_PDMA_CH));
...
static uint32_t BridgeSgpio_MakeDout(uint8_t sload, uint8_t sdata, uint8_t sclk)
{
    ...
    value = PA->DOUT & ~M031_SGPIO_OUT_MASK;
    if (sdata != 0u)
    {
        value |= BIT0;
    }
...
{
    if (g_sgpio.waveform_count >= M031_SGPIO_WAVEFORM_WORD_MAX)
    {
        return;
    }
...
static uint8_t BridgeSgpio_StartFrame(void)
{
    ...
    PA->DATMSK = (g_sgpio.saved_pa_datmsk | (~M031_SGPIO_OUT_MASK & 0xFFFFu)) & ~M031_SGPIO_OUT_MASK;
    BridgeSgpio_DriveIdle();
    ...
    PDMA_RESET(PDMA, M031_SGPIO_PDMA_CH);
    PDMA_Open(PDMA, (1ul << M031_SGPIO_PDMA_CH));
    PDMA_CLR_TD_FLAG(PDMA, (1ul << M031_SGPIO_PDMA_CH));
    PDMA_SetTransferCnt(PDMA, M031_SGPIO_PDMA_CH, PDMA_WIDTH_32, g_sgpio.waveform_count);
    PDMA_SetTransferAddr(PDMA, M031_SGPIO_PDMA_CH,
                         (uint32_t)&g_sgpio_waveform[0], PDMA_SAR_INC,
                         (uint32_t)&PA->DOUT, PDMA_DAR_FIX);
    PDMA_SetTransferMode(PDMA, M031_SGPIO_PDMA_CH, M031_SGPIO_PDMA_REQ, 0u, 0u);
    PDMA_SetBurstType(PDMA, M031_SGPIO_PDMA_CH, PDMA_REQ_SINGLE, PDMA_BURST_1);

    half_rate_hz = g_sgpio.clock_hz * 2u;
    actual_rate_hz = TIMER_Open(M031_SGPIO_TIMER, TIMER_PERIODIC_MODE, half_rate_hz);
    (void)actual_rate_hz;
    TIMER_SetTriggerSource(M031_SGPIO_TIMER, TIMER_TRGSRC_TIMEOUT_EVENT);
    TIMER_SetTriggerTarget(M031_SGPIO_TIMER, TIMER_TRG_TO_PDMA);
    TIMER_ClearIntFlag(M031_SGPIO_TIMER);

    g_sgpio.frame_active = 1u;
    TIMER_Start(M031_SGPIO_TIMER);
    return 1u;
}

uint8_t M031BridgeSgpio_Config(uint8_t slot_count, uint32_t clock_hz)
{
...
uint8_t M031BridgeSgpio_Apply(uint8_t enable, uint8_t periodic, uint16_t interval_ms, uint8_t sload_raw,
...
uint8_t M031BridgeSgpio_Off(void)
{
    BridgeSgpio_EnsureReady();
    BridgeSgpio_StopFrame();
    g_sgpio.enabled = 0u;
...
uint8_t M031BridgeSgpio_GetStatus(uint8_t *enabled, uint8_t *periodic, uint8_t *slot_count,
                                  ...
                                  uint32_t *sdata_in_words)
{
    ...
    if (sdata_in_words != 0)
    {
        for (i = 0u; i < M031_SGPIO_CAPTURE_WORD_MAX; ++i)
        {
            sdata_in_words[i] = 0u;
        }
    }
...
void M031BridgeSgpio_Process(void)
{
    uint32_t now;
